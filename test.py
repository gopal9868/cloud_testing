...
with open(input_path + 'Table_List_Compare.txt', 'r') as tabf1:
    ...
    for line in tabf1:
        ...
        with  open(output_path + cloud_com_result, 'w') as f23:
            pass
        # print(a_dict)
        while (si < len(a_dict)):
            if '_TS' in a_dict[si]:
        ...
        # with open(output_path + onprem_com_query, 'w') as premf1, open(output_path + cloud_com_query, 'w') as cf1:
        prem_query_line = 'SELECT ' + new_primary_key_input1 + new_select_column_input1.rstrip(
            ',') + ' FROM ' + onprem_db_name + '.' + onprem_schema_name + '.' + onprem_table_name + ' where ' + filter_fields_input + '>=' + "'" + filter_min_value_input + "'" + ' and ' + filter_fields_input + '<' + "'" + filter_max_value_input + "'" + ' and ' + filter_value_input + ' order by ' + order_by_input + ' fetch first ' + str(
            no_of_rows_input) + ' rows only'
        #   premf1.write(prem_query_line)
        print(prem_query_line)
        cloud_query_line = 'SELECT ' + new_primary_key_input1 + new_select_column_input1 + "'" + tabschema_input + "','" + tabname_input + "'," + "'" + file_name_com_2 + "','" + primary_key_input + "'," + " current_timestamp::timestamp_ntz FROM " + cloud_db_name + '.' + tabschema_input + '.' + tabname_input + ' where ' + filter_fields_input + '>=' + "'" + filter_min_value_input + "'" + ' and ' + filter_fields_input + '<' + "'" + filter_max_value_input + "'" + ' and ' + filter_value_input + ' order by ' + order_by_input + ' limit ' + str(
            no_of_rows_input)
       ...
       # prem_ins_query1 = """
        ...
        #prem_del_query1 = """
          #               delete from cns1p.LGCY_IRM_DM.TESTING_DATA_SOURCE where file_name='{file_name3}'""".format(
       ...
       # cloud_ins_query1 = """
           ...
           #           {col_list1} SCHEMA_NAME,TABLE_NAME,file_name,PRIMARY_KEY_COLUMN,PROCESS_TS ) {query_str_cloud1}""".format(
           # col_list1=col_list, query_str_cloud1=cloud_query_line)
        #cloud_del_query1 = """
         #                    delete from cns1p.LGCY_IRM_DM.TESTING_DATA_TARGET where file_name='{file_name3}'""".format(
        #    file_name3=file_name_com_2)
        ...
        compare_query_ins1 = """insert into CNS1p.LGCY_IRM_DM.TESTING_RESULT_COMPARE
                      ...
                      """.format(schema_name1=tabschema_input, table_name1=tabname_input,
                                 ...
                                 filter_max_value_input1=filter_max_value_input, pk_join1=pk_join, pk_concat1=pk_concat,
                                 cnt1=cnt)
